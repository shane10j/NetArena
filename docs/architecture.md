...
- `planner` produces Kubernetes investigation and action plans.
- `verifier` reviews the coordinator draft for missing evidence or unsafe assumptions.
...
If no model is configured, the agent returns a deterministic scaffold response so A2A conformance
checks and Docker startup still work.
